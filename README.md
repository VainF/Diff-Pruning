...
Our original exp code based on DDIM can be found at [exp_code](exp_code). There is still some performance issues with the diffuser trainer, which leads to lower FID than the reported (~7 for cifar). We will fix this issue as soon as possible.
